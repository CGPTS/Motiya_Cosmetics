...
const SERVICE_DURATIONS = Object.freeze(Object.fromEntries(SERVICE_CATALOG.map(service => [service.name, service.duration])));
...
function getSelectedServiceIds() {
  ...
  if (checked.length > 0) return uniqueServiceIds(checked);
  const stored = AppState?.getOrder?.().serviceIds || orderData.serviceIds || [];
  if (stored.length > 0) return uniqueServiceIds(stored);
  return serviceIdsFromEventType(document.getElementById('eventType')?.value ?? '');
}

function setSelectedServiceIds(ids, { preserveSlot = false } = {}) {
  ...
  document.querySelectorAll('input[name="serviceIds"]').forEach(input => {
    ...
    input.closest('.service-choice')?.classList.toggle('selected', input.checked);
  });
  updateEventTypeHidden(unique);
  syncServiceSelectionState(unique);
  if (!preserveSlot && calendarManager) calendarManager.clearSelection?.();
}

function updateEventTypeHidden(ids = getSelectedServiceIds()) {
  const hidden = document.getElementById('eventType');
  if (hidden) hidden.value = serviceNamesFromIds(ids).join(', ');
}
...
function getServicePrice(eventType) {
  return serviceTotals(serviceIdsFromEventType(eventType)).price;
}

function getServiceDuration(eventType) {
  return serviceTotals(serviceIdsFromEventType(eventType)).duration;
}
...
function syncServiceSelectionState(ids = getSelectedServiceIds()) {
  const services = serviceSnapshotFromIds(ids);
  const totals = serviceTotals(ids);
  const eventType = serviceNamesFromIds(ids).join(', ');
  const totalEl = document.getElementById('selectedServicesTotal');
  if (totalEl) {
    totalEl.textContent = services.length
      ? `${services.length} טיפולים נבחרו · ${formatDuration(totals.duration)} · ${formatPrice(totals.price)}`
      : 'בחרי לפחות טיפול אחד';
  }
  AppState.setOrder({
    serviceIds: uniqueServiceIds(ids),
    services,
    eventType,
    price: totals.price,
...
function updateServicePricePreview() {
  ...
  const selected = serviceNamesFromIds(ids);
  const totals = serviceTotals(ids);
  const box = document.getElementById('selectedServicePrice');
  if (box) {
    box.innerHTML = selected.length
      ? `<span>הטיפולים שבחרת</span><strong>${selected.join(' + ')}</strong><b>${formatPrice(totals.price)}</b><small>${formatDuration(totals.duration)}</small>`
      : '<span>בחרי לפחות טיפול אחד בשלב הקודם כדי לראות כאן מחיר ומשך.</span>';
  }
  document.querySelectorAll('.service-price-card').forEach(card => {
    card.classList.toggle('selected', ids.includes(card.dataset.serviceId));
  });
  const pricing = document.querySelector('input[name="pricing"]');
  if (pricing) { pricing.dataset.price = String(totals.price); pricing.dataset.basePrice = String(totals.price); pricing.checked = true; }
  AppState.setOrder({ residentType: 'client', residentText: 'לקוחה' });
  orderData.residentType = 'client';
  orderData.residentText = 'לקוחה';
  syncServiceSelectionState(ids);
}
...
const AppState = (() => {
  ...
  return Object.freeze({
    ...
    reset() {
      ...
      _state.order = {
        firstName:'',lastName:'',phone:'',eventType:'',serviceIds:[],services:[],notes:'',
        residentType:'client',residentText:'לקוחה',price:0,totalDuration:0,projector:false,calendarInfo:null,
      };
    },
  });
})();
...
function _serializeCalendarInfo(calInfo) {
  ...
  return {
    ...
    startMinutes: calInfo.startMinutes,
    endMinutes:   calInfo.endMinutes,
    durationMinutes: calInfo.durationMinutes,
...
function _calendarInfoFromDraft(saved) {
  ...
  const startMinutes = Number.isInteger(saved.startMinutes) ? saved.startMinutes : slotToMinutes(saved.slot);
  const durationMinutes = saved.durationMinutes || saved.totalDuration || getSelectedServiceIds().reduce((sum, id) => sum + (SERVICE_BY_ID[id]?.duration || 0), 0) || SLOT_STEP_MINUTES;
  const meta = buildSlotMeta(startMinutes, durationMinutes);
  ...
  return {
    ...
    startMinutes,
    endMinutes:  saved.endMinutes || meta.endMinutes,
    durationMinutes,
    gregDate:    greg,
    hebrewDate:  heb,
    dayName,
...
function isValidDraftSlot(slot) {
  return slotToMinutes(slot) !== null;
}
...
function _hasMeaningfulDraft(draft) {
  const step = parseInt(draft?.currentStep, 10) || 1;
  return step >= 3;
}
...
function saveDraft() {
  ...
  const step = AppState.get('currentStep') || currentStep || 1;
  if (step < 3 || step >= CONFIG.TOTAL_STEPS || AppState.get('bookingSaved')) {
    clearDraft();
    return;
  }
...
function getSavedDraft() {
  ...
  try {
    ...
    if (draft?.version === CONFIG.DRAFT_VERSION && _hasMeaningfulDraft(draft)) return draft;
    clearDraft();
    return null;
  } catch (_) {
    clearDraft();
...
function restoreDraft(draft) {
  ...
  setSelectedServiceIds(f.serviceIds?.length ? f.serviceIds : serviceIdsFromEventType(f.eventType), { preserveSlot: true });
  ...
  const restoredServiceIds = f.serviceIds?.length ? f.serviceIds : serviceIdsFromEventType(f.eventType);
  const normalizedEventType = serviceNamesFromIds(restoredServiceIds).join(', ');
  const residentText = RESIDENT_LABELS[draft.residentType] ?? draft.residentType ?? '';
  const totals = serviceTotals(restoredServiceIds);
  ...
  AppState.setOrder({
    ...
    serviceIds: uniqueServiceIds(restoredServiceIds),
    services: serviceSnapshotFromIds(restoredServiceIds),
    notes:     f.notes || '',
  ...
  switch (step) {
    ...
    case 3: {
      ...
      if (serviceIds.length === 0) {
        Toast.warning('יש לבחור לפחות טיפול אחד כדי להמשיך');
        valid = false;
      }
      if (!valid) Toast.warning('יש לתקן את השדות המסומנים');
      if (valid) syncServiceSelectionState(serviceIds);
      return valid;
    }
    ...
    case 4: {
      const serviceIds = getSelectedServiceIds();
      if (serviceIds.length === 0) {
        Toast.warning('יש לבחור לפחות טיפול אחד מתוך המחירון');
        return false;
      }
      updateServicePricePreview();
      return true;
...
function populateSummary() {
  ...
  set('sumPhone', phone);
  set('sumEventType', services.map(service => `${service.name} (${formatPrice(service.price)}, ${formatDuration(service.duration)})`).join(' | '));
  ...
  set('sumDuration', formatDuration(totals.duration));
  set('sumTotal', formatPrice(totals.price));
  const review = document.getElementById('submitReviewCard');
  if (review) {
    review.innerHTML = `
      <strong>${firstName} ${lastName}</strong><br>
      ${services.map(service => service.name).join(' + ')}<br>
      ${calInfo ? `${calInfo.gregDate} | ${calInfo.hoursText}` : ''}<br>
      ${formatDuration(totals.duration)} · ${formatPrice(totals.price)}
    `;
  }
  ...
  const patch = {
    ...
    price: totals.price,
    totalDuration: totals.duration,
    residentText, residentType: 'client', projector, calendarInfo: calInfo,
  };
...
async function submitOrder() {
  ...
  const serviceIds = uniqueServiceIds(mergedOrder.serviceIds?.length ? mergedOrder.serviceIds : getSelectedServiceIds());
  const services = serviceSnapshotFromIds(serviceIds);
  const totals = serviceTotals(serviceIds);
  const normalizedEventType = serviceNamesFromIds(serviceIds).join(', ');
  const priceText = formatPrice(totals.price);

  if (serviceIds.length === 0) {
    Toast.error('יש לבחור לפחות טיפול אחד לפני שליחת ההזמנה');
    return false;
  }

  const startMinutes = Number.isInteger(calInfo.startMinutes) ? calInfo.startMinutes : slotToMinutes(calInfo.slot);
  const endMinutes = startMinutes + totals.duration;
  const workday = calInfo.date ? workingHoursForDow(calInfo.date.getDay()) : null;
  if (!Number.isInteger(startMinutes) || !workday || endMinutes > workday.end) {
    Toast.error('שעת התור אינה זמינה למשך הטיפולים שנבחרו. חזרי ללוח השנה ובחרי שעה אחרת.');
    return false;
  }
  ...
  try {
    ...
    if (!AppState.get('bookingSaved')) {
      ...
      const lockDocs = buildSlotLocks(calInfo.dateKey, calInfo.slot, orderNumber, startMinutes, endMinutes);
      const probeRefs = lockProbeSlotsForRange(startMinutes, endMinutes)
        .map(slot => db.collection('booked_slots').doc(`${calInfo.dateKey}_${slot}`));
      const bookingRef = db.collection('bookings').doc(orderNumber);

      Logger.info('Saving booking (atomic)…', { orderNumber, lockCount: lockDocs.length });
      try {
        await withRetry(() => db.runTransaction(async (tx) => {
          for (const ref of probeRefs) {
            const snap = await tx.get(ref);
            if (snap.exists && snap.data()?.orderNumber !== orderNumber) {
              throw new Error('SLOT_TAKEN');
            }
          }
          tx.set(bookingRef, bookingData);
          lockDocs.forEach(lock => {
            tx.set(db.collection('booked_slots').doc(lock.id), lock.data);
          });
        }));
      } catch (saveError) {
    ...
    const emailParams = {
      ...
      'טיפולים':             normalizedEventType,
      'סיווג':               mergedOrder.residentText || 'לקוחה',
      'תאריך':               `יום ${calInfo.dayName} | ${calInfo.gregDate} | ${calInfo.hebrewDate}`,
      'שעה':                 `${minutesToTime(startMinutes)} (${minutesToTime(startMinutes)} - ${minutesToTime(endMinutes)})`,
      'משך כולל':            formatDuration(totals.duration),
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('input[name="serviceIds"]').forEach(input => {
    input.addEventListener('change', () => {
      setSelectedServiceIds(getSelectedServiceIds());
      syncMemorialPricingOptions();
      const step = AppState.get('currentStep') || currentStep;
      if (step >= 4) syncStep4PricingUI();
      if (step >= 6) populateSummary();
    });
  });

  syncMemorialPricingOptions();
  syncServiceSelectionState([]);
