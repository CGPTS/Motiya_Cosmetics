...
function initEmailJS() {
  if (!window.emailjs) {
    debugLog("warn", "EmailJS SDK missing on window");
    return;
  }
  if (typeof EMAILJS_CONFIG === "undefined") {
    debugLog("warn", "EMAILJS_CONFIG is undefined");
    return;
  }
  if (!EMAILJS_CONFIG.publicKey || EMAILJS_CONFIG.publicKey.includes("PASTE_")) return;
  emailjs.init(EMAILJS_CONFIG.publicKey);
  debugLog("log", "EmailJS initialized", {
    serviceId: EMAILJS_CONFIG.serviceId,
    templateId: EMAILJS_CONFIG.templateId,
    ownerEmail: EMAILJS_CONFIG.ownerEmail
  });
}
...
async function submitOrder() {
  ...
  try {
    ...
    try {
      ...
      if (emailResult?.sent) {
        debugLog("log", "EmailJS send success", {
          orderNumber: payload.orderNumber,
          status: emailResult.status,
          text: emailResult.text
        });
      } else {
        debugLog("warn", "EmailJS skipped", emailResult);
      }
    } catch (mailError) {
      debugLog("error", "EmailJS send failed", {
        message: mailError?.message,
        text: mailError?.text,
        status: mailError?.status
      });
      const extra = mailError?.text || mailError?.message || "unknown_error";
      Toast.warning(`התור נשמר, אבל המייל לא נשלח (${extra}). בדקי Allowed Origins ו-Template Variables.`);
    }
...
async function sendBookingEmail(payload) {
  if (!window.emailjs) {
    return { sent: false, reason: "emailjs_sdk_missing" };
  }
  if (typeof EMAILJS_CONFIG === "undefined") {
    return { sent: false, reason: "emailjs_config_missing" };
  }
  if (!EMAILJS_CONFIG.serviceId || !EMAILJS_CONFIG.templateId) {
    return { sent: false, reason: "service_or_template_missing" };
  }
  if (EMAILJS_CONFIG.templateId.includes("PASTE_")) {
    Toast.info("תזכורת: יש להזין Template ID כדי להפעיל שליחת מייל.");
    debugLog("warn", "Email skipped: template id placeholder");
    return { sent: false, reason: "template_placeholder" };
  }
  ...
  const templateParams = {
    ...
    from_name: "מערכת תורים - סטודיו לק ג'ל",
    ...
    duration_min: String(payload.durationMin),
    price_text: formatPrice(payload.price)
  };
  ...
  debugLog("log", "EmailJS raw response", response);
