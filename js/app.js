...
async function sendBookingEmail(payload) {
  ...
  const htmlBody = `
<div dir="rtl" style="font-family:Arial,sans-serif;line-height:1.7;color:#3a0f25">
  <h2 style="margin:0 0 12px;color:#c4116b">הזמנת תור חדשה</h2>
  <p><strong>מספר הזמנה:</strong> ${payload.orderNumber}</p>
  <p><strong>שם לקוחה:</strong> ${payload.fullName}</p>
  <p><strong>טלפון:</strong> ${payload.phone}</p>
  <p><strong>טיפול:</strong> ${payload.service}</p>
  <p><strong>תאריך:</strong> ${dateText}</p>
  <p><strong>שעה:</strong> ${payload.appointmentTime}</p>
  <p><strong>משך:</strong> ${payload.durationMin} דקות</p>
  <p><strong>מחיר:</strong> ${formatPrice(payload.price)}</p>
</div>`.trim();
