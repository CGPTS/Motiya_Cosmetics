...
function initEmailJS() {
  if (!window.emailjs || !window.EMAILJS_CONFIG) return;
...
async function submitOrder() {
  ...
  try {
    ...
    try {
      ...
      debugLog("log", "EmailJS send success", { orderNumber: payload.orderNumber });
    } catch (mailError) {
...
async function sendBookingEmail(payload) {
  if (!window.emailjs || !window.EMAILJS_CONFIG) return;
  if (!EMAILJS_CONFIG.serviceId || !EMAILJS_CONFIG.templateId) return;
