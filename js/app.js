...
function initLoopVideoById(videoId, label) {
  ...
  const tryPlay = () => {
    ...
    if (p && typeof p.catch === "function") {
      p.catch((err) => debugLog("warn", `${label} could not autoplay`, err?.message || err));
    }
  };

  video.addEventListener("error", () => {
    debugLog("error", `${label} failed to load`, video.currentSrc || "videos/logo.mp4");
  });
  video.addEventListener("loadedmetadata", tryPlay, { once: true });
  video.addEventListener("canplay", tryPlay, { once: true });
  tryPlay();

  // iOS / in-app browsers sometimes require one user gesture
...
function initLogoLoopVideos() {
  initLoopVideoById("logoLoopVideo", "Header logo loop video");
  initLoopVideoById("successLogoVideo", "Success logo loop video");
}

function shouldWarnBeforeUnload() {
  return AppState.currentStep >= 2 && AppState.currentStep < 6 && !AppState.isSubmitting;
}
...
function initUnloadProtection() {
  ...
  window.addEventListener("beforeunload", (event) => {
    ...
    // Modern browsers show a built-in warning text.
    event.returnValue = "";
  });

  // Custom confirmation for keyboard refresh shortcuts.
  document.addEventListener("keydown", (event) => {
    const isRefreshKey = event.key === "F5";
    const isReloadCombo = (event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "r";
    if (!isRefreshKey && !isReloadCombo) return;
    if (!shouldWarnBeforeUnload()) return;

    event.preventDefault();
    const ok = window.confirm("לרענן את הדף? ההתקדמות בהזמנה תימחק ותחזרי להתחלה.");
    if (ok) {
...
function showStep(step) {
  ...
  if (target === 1) initLoopVideoById("logoLoopVideo", "Header logo loop video");
  if (target === 6) initLoopVideoById("successLogoVideo", "Success logo loop video");
}
...
function validateStep(step) {
  ...
  switch (step) {
    ...
    case 2: {
      ...
      if (!phone) { setFieldError("phone", "שדה חובה"); valid = false; }
      if (!valid) {
